...
public enum ParticleEffect {
    ...
    public static void sendToPlayer(ParticleEffect effect, Player player, Location location, float offsetX, float offsetY, float offsetZ, float speed, int count) throws Exception {
        Packet63WorldParticles packet = createPacket(effect, location, offsetX, offsetY, offsetZ, speed, count);
    ...
    public static Packet63WorldParticles createPacket(ParticleEffect effect, Location location, float offsetX, float offsetY, float offsetZ, float speed, int count) throws Exception {
        if (count <= 0)
            count = 1;
        Packet63WorldParticles packet = getPacket63WorldParticles();
        Field[] f = packet.getClass().getDeclaredFields();
        setValue(packet, f[0].getName(), effect.name);
        setValue(packet, f[1].getName(), (float) location.getX());
        setValue(packet, f[2].getName(), (float) location.getY());
        setValue(packet, f[3].getName(), (float) location.getZ());
        setValue(packet, f[4].getName(), offsetX);
        setValue(packet, f[5].getName(), offsetY);
        setValue(packet, f[6].getName(), offsetZ);
        setValue(packet, f[7].getName(), speed);
        setValue(packet, f[8].getName(), count);
        return packet;
    }
    ...
    private static void setValue(Object instance, String fieldName, Object value) throws Exception {
        ...
        field.set(instance, value);
        	...
        	for(int i = 0; i <= f.length; i++){
        		Monument.logger.info(f[i].getName());
        	}
        }
    }
    ...
    private static Packet63WorldParticles getPacket63WorldParticles() throws Exception {
